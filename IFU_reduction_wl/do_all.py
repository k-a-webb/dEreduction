    ...
    with open(XYSN_FILE, 'w') as outfile:
        for i in range(sci_yaxis - 1):
            for j in range(sci_xaxis - 1):
                outfile.write('   {}   {}   {}   {}\n'.format(i, j, sci_data[i, j], np.sqrt(var_data[i, j])))
...
def bin_spectra():
    ...
    v2b_output = pd.read_table(V2B_FILE, sep=r"\s*", engine='python',
                               names=["x", "y", "bin"])  # , skiprows=1)
...
def combine_spectra():
    ...
    for i in range(np.amax(v2b_output['bin'].values) + 1):
        ...
        assert os.path.exists("{}/{}".format(DIR_SCI, file_sci_i)), "File {}/{} does not exist".format(DIR_SCI,
                                                                                                       file_sci_i)
        assert os.path.exists("{}/{}".format(DIR_VAR, file_var_i)), "File {}/{} does not exist".format(DIR_VARa,
                                                                                                       file_var_i)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(DIR_SCI, FILE_VAR.format(0))):
        print('>>>>> Binning spectra')
        bin_spectra()
    if not os.path.exists(os.path.join(DIR_SCI, BIN_VAR.format(0))):
