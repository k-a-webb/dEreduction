...
def fit_spectra(bin_sci_j, ppxf_bestfit_j, plot=False):
    ...
    emlns, lnames, lwave = emission_lines(logLam1, [wmin, wmax], 2.3)
    ...
    xOIII, abs_fit_OIIIa = fit_gaussian_lorentz_lincont(galaxy, iOIII, bestfit, log_bins)
...
def fit_gaussian_lorentz_lincont(galaxy, iline, bestfit, log_bins):
    ...
    if 5042 in range(int(x[0]), int(x[-1])):  # apply mask to region with weird bump that bugs fitting method
        ...
        popt2, pcov2 = curve_fit(make_gaussian_lorentz_wbflin(bf_lin_ma_data), x_ma_data, cutout2_ma_data,
...
def clean_spec_30(bin_sci):
    ...
    assert os.path.exists(
        bin_spec_features), 'Features fits file has not yet been created, refer to instructions in clean_spec_30'
